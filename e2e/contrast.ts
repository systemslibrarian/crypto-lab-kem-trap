 ...
 *
 *  - text over a TRANSLUCENT surface. This page draws no gradients at all, but
 *    almost every state colour it paints is a `color-mix(in oklab, ..., 
 *    transparent)` wash: `.candidate.is-chosen`, `.oracle-table td.oracle-now`,
 *    `.byte-diff`, `.cl-hero-why`. A wash that mixes toward `transparent` is a
 *    translucent fill, so the text on it composites onto whatever the wash is
 *    sitting on — the card, then the panel, then the page — rather than onto a
 *    flat colour axe can read off one element.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. This lab is deliberate about
 *    that (`.candidate.is-dropped` sets `opacity: 1` and dims with `color`
 *    instead, which is the right call), but `@keyframes step-in` still runs the
 *    newly revealed FO step up from `opacity: 0.25`, and every panel re-renders
 *    on every control press.
 *
 ...
 * rather than against white. A gradient, where one exists, is SAMPLED at the
 * text's own location rather than judged at its worst stop — see `sampleLayer`.
 *
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. Every `.bytes`
 *    view is an `overflow: auto` box capped at `max-height: 8.5rem` (7rem below
 *    640px) holding up to 32 wrapped hex cells, and the `.code` block scrolls
 *    the same way. Content scrolled past the client box is not dimmed or partly
 *    drawn — it is absent from the frame, and asking what colour it sits on has
 *    no answer. Its rect is below every ancestor's box, so the ancestor walk
 *    would find nothing behind it and fall through to white, inventing a
 *    failure for hex nobody can see.
 *
 ...
 *    draws no SVG text — its only SVG is the shared header's icon set, which
 *    carries none — but the rule is kept because it is cheap and silent when
 *    unused: an `<svg>` draws a shape as a `<rect>` and then its
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     *
     * A hand-rolled `rgba()` regex is not enough here: every accented surface on
     * this page is `color-mix(in oklab, ...)` — the controls panel, the chosen
     * FO candidate, the active oracle cell, the changed-byte highlight, the
     * accept-bordered caller card — and Chromium reports those to
     ...
     * fabricates failures (dark-theme hex read as sitting on white) and, far
     ...
     *
     * This page ships no gradients, so on today's stylesheet this path always
     * takes the `TRANSPARENT` early return. It is kept because it is silent when
     * unused and because the alternative it replaced was actively wrong: an
     * older form of this helper judged every gradient at its WORST STOP,
     * assuming that stop covered the text wherever the text sat. That holds for
     * a gradient spanning its element and fails for a small positioned one — in
     * one lab it invented a 3.56:1 reading for footer links that render ~4.9:1.
     ...
     *
     * This page renders a `<span>` per hex byte across a dozen byte views —
     * several hundred text-owning elements — and every one of them walks the
     * same handful of ancestors. Without
     ...
     * surface it happens to sit on and INVENTS failures. This lab's `.vh` is
     * exactly the classic recipe (`clip: rect(0 0 0 0)`), and it carries the
     * lab's single live-region announcer, every changed-byte "byte N changed"
     * note, every table caption and the "(this ciphertext)" marker on the active
     * oracle cell — so without this guard a single bit flip would add dozens of
     * fabricated findings. Deliberately narrow — only a ZERO-area clip
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This page has no `<details>`, but the FO panel's
      // unrevealed steps carry the `hidden` attribute, which this catches for
      // the same reason: the gate reveals them by pressing the lab's own button,
      // never by stripping the attribute from script.
      ...
      // the shared header's `.cl-skip-link`, which parks at `top: -3rem` and
      // slides into view only on focus. Measuring the parked copy invents a
     ...
     * stricter than axe on decorative content. Honour the same boundary. Here
     * that covers every `aria-hidden` verdict glyph (the indicator icons, the
     * chosen-candidate "▶", the leaks/uniform ticks in the oracle table), each
     * of which sits beside a word carrying the same meaning.
     */
